...
| AEB-DHW-001 | Autonomous ACS/EHW policy | autonomy strategy + closed safe writer | M63 active; EQ 3018 ES/manual E7000670C documented. Runtime audit confirmed telemetry, disabled writer and false-surplus protection. Shadow bounds 45/50/55 degC approved. Synchronized REST/log test: `SILENT_POLL_FAILURE` / `TRANSPORT_DEGRADED`. Branch-only hardening candidate preserves entity IDs, reduces polling and makes readiness age-aware; static tests pass; package remains undeployed. | M63 | COVERED | Publish/review the uncommitted candidate through an authorized Git working copy or connector, then obtain separate deploy authorization. Runtime g01..g04 remain mandatory for LIVE/treatment authority only. | Review the two-package diff and runbook; if approved, publish it without deploying. Runtime deploy/restart and shadow activation remain separate gates. | P1 |
