...
| AEB-DHW-001 | Autonomous ACS/EHW policy | autonomy strategy + closed safe writer | M63 active; EQ 3018 ES/manual E7000670C documented. Read-only runtime audit 2026-09-24 confirmed telemetry, dry-run and disabled writer, plus a false-surplus example during measured grid import. Shadow bounds 45/50/55 degC approved; package remains undeployed. | M63 | COVERED | Validate `last_reported` freshness. Runtime g01..g04 remain mandatory for LIVE/treatment authority, not for installing a non-actuating observer. | Validate freshness across one EHW and SDM120 poll, then request separate authorization for shadow-only deployment. | P1 |
