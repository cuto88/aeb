...
| AEB-DHW-001 | Autonomous ACS/EHW policy | autonomy strategy + closed safe writer | M63 active; installed EQ 3018 ES and model-specific manual E7000670C are documented. Policy contract and non-actuating shadow package drafted; current g01..g04 values, approved comfort/preheat bounds and runtime freshness remain open, therefore the policy is safe-blocked. | M63 | COVERED | Runtime g01..g04 + approved comfort/preheat bounds + freshness evidence | Read current g01..g04 without writes, approve comfort/preheat bounds and verify input freshness; then deploy shadow-only package with both verification gates still off. | P1 |
