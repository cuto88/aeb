...
| AEB-DHW-001 | Autonomous ACS/EHW policy | autonomy strategy + closed safe writer | M63 transport hardening `TRANSPORT_HARDENING_PASS` on 2026-09-25: commit `d65fc19`, one restart, five raw advances each, max cadence ~180.2 s, simultaneous freshness/readiness/derived/SDM120 PASS, writer safety PASS, Modbus writes 0. Shadow bounds 45/50/55 degC remain shadow-only. Static shadow audit requires the isolated null-safe/freshness/reason-code patch; shadow remains undeployed. | M63 | COVERED | Run static tests and Quality Gates for the shadow-only patch, then obtain separate shadow deployment authorization. g01..g04 and PM4/accounting remain LIVE/economic gaps. | Do not deploy or activate shadow until `SHADOW_PACKAGE_READY` is reached; never promote LIVE from this evidence alone. | P1 |
