...
| AEB-DHW-001 | Autonomous ACS/EHW policy | autonomy strategy + closed safe writer | M63 active; EQ 3018 ES/manual E7000670C documented. Read-only runtime audit 2026-09-24 confirmed telemetry, dry-run and disabled writer, plus a false-surplus example during measured grid import. Shadow bounds 45/50/55 degC approved; package remains undeployed. Recorder freshness follow-up was inconclusive because nullable timestamps were initially misinterpreted. | M63 | COVERED | Validate live state-object `last_reported`; Recorder SQL is not authoritative for unchanged reports. Runtime g01..g04 remain mandatory for LIVE/treatment authority only. | From DS-XPS read live `last_reported` across one EHW poll; if PASS, request separate authorization for shadow-only deployment. | P1 |
