...
| AEB-DHW-001 | Autonomous ACS/EHW policy | autonomy strategy + closed safe writer | M63 active; installed EQ 3018 ES is already documented in the digital twin. Policy contract and non-actuating shadow package drafted; model-specific safety bounds and runtime evidence remain open, therefore the policy is safe-blocked. | M63 | COVERED | Verified EQ 3018 ES comfort/legionella bounds + runtime evidence | Reconcile a model-specific EQ 3018 ES source, verify native legionella authority/configuration, comfort/preheat bounds and input freshness; then deploy shadow-only package with both verification gates still off. | P1 |
