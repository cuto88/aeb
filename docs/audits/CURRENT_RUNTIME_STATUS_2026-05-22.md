# Current Runtime Status (2026-05-22)

## Scope
- Punto di ingresso unico per lo stato repo/runtime corrente dopo il burn-in operativo e l'ultimo consolidamento del trail.
- Questo documento aggiorna il quadro operativo precedente con l'evidenza giornaliera raccolta il `2026-05-22`.
...
- VMC delta UR policy tuning: `SOURCE UPDATED / runtime verify pending`
...
- `phase7_executive_status` on `2026-05-22` correctly reported `task_state=Running` as `IN_PROGRESS_PREVIOUS_RESULT`, not as a failure.

### Supervisor bridge
- `STEP110_AEB_SUPERVISOR_BRIDGE_CLOSURE_2026-05-15.md` closed the AEB supervisor bridge block.
- The host bridge is reachable from `n8n` and the canonical payload write path remains available.
...
- VMC `P1_delta_ur` now uses a dedicated hysteresis helper plus a thermal veto in source; runtime verify remains pending until reload.

### Source-side realignment
- The planner/arbiter naming drift has been closed on the source side.
- The runtime confirmation has now been observed and closed as well.

### UI / notification ergonomics
- The envelope and ventilation notification controls are now exposed as explicit opt-in flags in the dashboard trail.
...
## Use rule
- Use this file as the default entrypoint when the question is "what is the current runtime/repo posture after the 2026-05-22 burn-in closure?"
- Use `CURRENT_RUNTIME_STATUS_2026-05-21.md` when you need the immediately previous baseline snapshot.
